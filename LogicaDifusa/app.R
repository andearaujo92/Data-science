...
sets_options(option = "universe",value = seq(1,100,1))
...
# Criando o Sistema de Inferencia -----------------------------------------

sistema <- fuzzy_system(variaveis, regras)

# Definição da interface do usuario ---------------------------------------
...
ui <- fluidPage(
    ...
    fluidRow(
      column(6,h1("Sistema:"), plotOutput(outputId = "GrafSistema")),
      column(6, actionButton(inputId = "Processar",label = "Processar"),
